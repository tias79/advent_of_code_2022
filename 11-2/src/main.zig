...
pub fn main() anyerror!void {
    ...
    var inspections = std.AutoHashMap(u8, u32).init(allocator);

    var least_common_denominator : u64 = 1;
    for (monkies2) |monkey| {
            least_common_denominator *= monkey.div_test.divide_by;
    }
        ...
        while (i < monkies2.len) {
            ...
            for (monkey.items.items) |item| {
                var factor1 = switch (monkey.operation.factor1) {
                    Factor.old => item % least_common_denominator,
                    Factor.value => |v| v
                };
